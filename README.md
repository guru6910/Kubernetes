...
- when you use a StatefulSet with PersistentVolume (PV) and PersistentVolumeClaim (PVC), each pod typically gets its own PVC and PV. This behavior is by design to ensure that each pod in the StatefulSet has its own isolated storage.
...
- ConfigMap is used to store non-sensitive configuration data such as configuration files, command-line arguments, and environment variables.
- Using ConfigMap allows you to separate configuration artifacts from image content to keep containerized applications portable.
- ConfigMaps are namespaced resources, meaning they exist within a specific namespace and are accessible only to resources within that namespace unless explicitly shared.
- ConfigMap can be mounted as volumes in Pods, allowing applications to read configuration data from the filesystem.


## ${\color{red} \textbf{Secret}}$
- Secrets are used to store sensitive information such as passwords, OAuth tokens, SSH keys, and TLS certificates.
- Data in a Secret is stored in base64-encoded format, ensuring that it is handled as binary data, even though it is not encrypted by default.
